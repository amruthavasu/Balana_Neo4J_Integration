...
import org.wso2.balana.neo4j.ExecuteQuery;
...
public class FileBasedPolicyFinderModule extends PolicyFinderModule {
    ...
    public PolicyFinderResult findPolicy(EvaluationCtx context) {
    	...
    	AbstractRequestCtx requestCtx = context.getRequestCtx();
    	Set<Attributes> attributeSet = requestCtx.getAttributesSet();
    	Map<String, String> requestAttributes = new HashMap<String, String>();
    	for (Attributes attributes : attributeSet) {
    		for (Attribute attribute : attributes.getAttributes()) {
    			String category = attribute.getId().toString().split(":")[6];
    			String value = attribute.getValue().encode();
    			String id = attribute.getId().toString();
    			if (category.equals("conformance-test")) {
    				requestAttributes.put("environment", id);
    			} else {
    				requestAttributes.put(category, value);
    			}
    		}
    	}
    	
    	// Get policy file names from Neo4J 
    	//ExecuteQuery.test();
    	List<String> policyFiles = ExecuteQuery.execute(requestAttributes);
    	for (String file : policyFiles) {
    		loadPolicy(file, finder);
    	}
